...
export default function WoodCatalog() {
  ...
  return (
    <section id="catalog" className="py-16 md:py-28" style={{ background: 'var(--color-bg-subtle)', fontFamily: 'var(--font-display)' }}>
      <div className="px-5 sm:px-8 md:px-16 lg:px-24 max-w-screen-xl mx-auto">
        ...
        <h2 className="font-bold text-2xl sm:text-3xl md:text-4xl mb-8 md:mb-10"
        ...
        <div className="flex flex-wrap gap-2 mb-8 md:mb-10">
          {CATEGORIES.map(({ key, label }) => (
            <button key={key} onClick={() => setActive(key)}
              className="px-4 md:px-5 py-2 text-xs md:text-sm font-semibold uppercase tracking-[0.08em] border transition-all"
              style={{
          ...
          {filtered.map(({ slug, title, short, photo }) => (
            <Link key={slug} href={`/wood/${slug}`}
              ...
              <div className="flex flex-col gap-2 p-4 md:p-5 flex-1">
                ...
                <p className="text-xs hidden sm:block" style={{ color: 'var(--color-text-muted)' }}>{short}</p>
                <span className="mt-auto pt-2 text-[10px] md:text-xs font-semibold uppercase tracking-[0.1em]"
        ...
        <p className="text-xs md:text-sm text-center" style={{ color: 'var(--color-text-muted)' }}>
